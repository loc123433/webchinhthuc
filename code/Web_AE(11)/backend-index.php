		...
		<div class='item active'>
			...
			<div class='container'>
				<div class='carousel-caption'>
					<p><a class='btn btn-md btn-primary' href='order.php?masp=<?php echo $row['masp'] ?>' role='button'>Đăng ký</a></p>
				</div>
			</div>
		</div>
		<?php } else { ?>
...
function get_buy_the_most(){
	$conn = connect();
	mysqli_set_charset($conn, 'utf8');
	$sql = "SELECT * FROM sanpham sp, danhmuckh dm WHERE sp.madm = dm.madm ORDER BY sp.luotxem DESC LIMIT 8";
	$result = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($result)) { ?>
...
function get_the_most_recent(){
	$conn = connect();
	mysqli_set_charset($conn, 'utf8');
	$sql = "SELECT * FROM sanpham sp, danhmuckh dm WHERE sp.madm = dm.madm ORDER BY sp.ngay_nhap DESC LIMIT 8";
	$result = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($result)) {?>
	<div class='product-container' onclick="hien_sanpham('<?php echo $row['masp']?>')">
		<a data-toggle='modal' href='sanpham.php?masp=<?php echo $row['masp'] ?>' data-target='#modal-id'>
...
function addtocart_action(){
	...
	if($_SESSION['rights'] == "user"){
		...
		$sql = "";
		if($pos != ""){
			$sql = "DELETE FROM quantam WHERE user_id = '".$_SESSION['user']['id']."' AND masp = '".$masp."'"; 
		} else {
			$sql = "INSERT INTO quantam VALUES ('".$_SESSION['user']['id']."','".$masp."','1')";
		}
		$result = mysqli_query($conn, $sql);
		if(!$result){echo "Loi them quan tam";}
	}
}
...
function like(){
	...
	$sql = "";
	if($pos != ""){
		$sql = "DELETE FROM baivietyeuthich WHERE user_id = '".$_SESSION['user']['id']."' AND masp = '".$masp."'"; 
	} else {
		$sql = "INSERT INTO baivietyeuthich VALUES ('".$_SESSION['user']['id']."','".$masp."')";
	}
	$result = mysqli_query($conn, $sql);
